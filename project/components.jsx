/* Shared primitives for the nAi prototype */
...
const Icon = ({ name, size = 16, strokeWidth = 1.75, className = "", style = {} }) => {
  ...
  switch (name) {
    case "shield":
      return <svg {...props}><path d="M12 3 4 6v6c0 5 3.5 8 8 9 4.5-1 8-4 8-9V6l-8-3Z"/></svg>;
    case "shield-check":
    ...
    case "wand":
      return <svg {...props}><path d="m15 4 1 2 2 1-2 1-1 2-1-2-2-1 2-1 1-2Z"/><path d="M3 21 14 10"/><path d="m17 7 1.5 1.5"/></svg>;
    case "image":
    ...
    case "flame":
      return <svg {...props}><path d="M12 22c4 0 7-3 7-7 0-3-2-5-3-6-1 2-2 3-4 3 0-3-1-5-3-7-1 4-5 6-5 10 0 4 3 7 8 7Z"/></svg>;
    case "zap":
    ...
    case "library":
      return <svg {...props}><path d="M3 19a2 2 0 0 1 2-2h14v4H5a2 2 0 0 1-2-2Z"/><path d="M3 5a2 2 0 0 1 2-2h14v14H5a2 2 0 0 1-2-2V5Z"/></svg>;
    case "home":
      return <svg {...props}><path d="m3 11 9-8 9 8v9a2 2 0 0 1-2 2h-4v-7h-6v7H5a2 2 0 0 1-2-2v-9Z"/></svg>;
    case "bell":
      return <svg {...props}><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10 21a2 2 0 0 0 4 0"/></svg>;
    case "crown":
      return <svg {...props}><path d="m2 19 2-11 5 5 3-7 3 7 5-5 2 11H2Z"/><path d="M2 21h20"/></svg>;
    case "mail":
    ...
    case "edit":
      return <svg {...props}><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5Z"/></svg>;
    case "eye":
    ...
    case "logo":
      return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" className={className} style={style}><rect x="2" y="2" width="20" height="20" rx="6" fill="currentColor"/><path d="M7.5 16V9.5l4 5V9.5M14.5 9.5h2.5M15.75 9.5V16" stroke="white" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>;
    case "dot":
      return <svg width={size} height={size} viewBox="0 0 24 24" className={className} style={style}><circle cx="12" cy="12" r="4" fill="currentColor"/></svg>;
    default:
      return null;
  }
};

const Card = ({ children, className = "", style = {}, padding = 24, hover = false, onClick }) => (
  <div
    onClick={onClick}
    className={className}
    style={{
...
const Pill = ({ tone = "neutral", icon, children, style = {} }) => {
  const tones = {
...
const Button = ({ variant = "primary", size = "md", icon, iconRight, children, onClick, disabled, style = {}, full }) => {
  const sizes = {
  ...
  const variants = {
    primary: {
      bg: "var(--text)", fg: "#fff", bd: "var(--text)", hover: "#020617",
      disabledBg: "#F1F5F9", disabledFg: "var(--text-2)", disabledBd: "#E2E8F0",
    },
    secondary: {
      bg: "var(--surface)", fg: "var(--text)", bd: "var(--border-strong)", hover: "var(--bg-sunken)",
      disabledBg: "#F1F5F9", disabledFg: "var(--text-3)", disabledBd: "var(--border)",
    },
    ghost: {
      bg: "transparent", fg: "var(--text-2)", bd: "transparent", hover: "var(--bg-sunken)",
      disabledBg: "transparent", disabledFg: "var(--text-3)", disabledBd: "transparent",
    },
    danger: {
      bg: "var(--danger)", fg: "#fff", bd: "var(--danger)", hover: "#DC2626",
      disabledBg: "#F1F5F9", disabledFg: "var(--text-2)", disabledBd: "#E2E8F0",
    },
    pro: {
      bg: "linear-gradient(180deg, #2563EB 0%, #1D4ED8 100%)",
      fg: "#fff", bd: "#1D4ED8", hover: "linear-gradient(180deg, #1D4ED8 0%, #1E40AF 100%)",
      disabledBg: "#F1F5F9", disabledFg: "var(--text-2)", disabledBd: "#E2E8F0",
    },
  };
  const v = variants[variant];
  // Resolved colors — explicit muted state instead of opacity so the disabled
  // button reads as "available later", not "broken".
...
const Progress = ({ value, max = 100, tone = "ok", height = 6, showLabel = false }) => {
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      {showLabel && (
        <div className="mono tnum" style={{ fontSize: 11, color: "var(--text-3)", marginTop: 4 }}>{pct.toFixed(0)}%</div>
      )}
    </div>
  );
};

const Ring = ({ value, max = 100, size = 96, stroke = 8, tone = "ok", children }) => {
  const pct = Math.min(100, Math.max(0, (value / max) * 100));
...
const Thumb = ({ label, size = 40, radius = 6, hue = 250 }) => (
...
/* Toggle — used in settings & autopilot rows */
const Toggle = ({ on, onChange, disabled = false, size = "md" }) => {
  const dims = size === "sm"
...
Object.assign(window, { Icon, Card, Pill, Button, Progress, Ring, Thumb, Divider, KBD, Tooltip, Toggle });
