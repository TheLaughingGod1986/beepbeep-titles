                    ...
                    {plan === 'pro' ? (
                        showQueue ? (
                            <>
                                ...
                                <span style={{ color: 'var(--text-2)', fontWeight: 500 }}>Autopilot active</span>
                            ...
                            <>
                                ...
                                <span style={{ color: 'var(--text-2)', fontWeight: 500 }}><span className="mono tnum">{dailyUsed}</span> pages improved today</span>
                            </>
                        )
                    ) : showQueue ? (
                        <>
...
const AutopilotUpsellCard = ({ onUpgrade }) => (
    <Card padding={0}>
        <div style={{ padding: '18px 22px 14px' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, letterSpacing: '-0.01em', marginBottom: 4 }}>Automatically optimise new pages in the background.</div>
        </div>
        <div style={{ padding: '2px 22px 14px', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {['New pages covered instantly', 'Weekly progress summary', 'WooCommerce support'].map( ( t, i ) => (
        ...
        <div style={{ padding: '18px 22px 12px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12.5, color: 'var(--text-2)', marginTop: 4 }}>
                    {autoOptimise ? 'New pages get title & meta the moment they\'re added.' : 'Re-enable to keep new pages covered automatically.'}
                </div>
            </div>
...
const FooterMetrics = ({ streak, dailyUsed, dailyLimit, monthlyUsed, monthlyLimit, plan }) => {
    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', padding: '14px 4px 4px', gap: 40, flexWrap: 'wrap', borderTop: '1px solid var(--hairline)', marginTop: 20 }}>
            ...
            {plan === 'pro' ? (
                <>
                    <Item label="Improvements this week" value={<><span className="mono tnum">{monthlyUsed}</span> pages</>}/>
                    <Item label="Autopilot" value={<span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><span className="pulse-dot" style={{ width: 6, height: 6, background: 'var(--ok)' }}/>Active · monitoring new pages</span>}/>
                </>
            ) : (
                <>
