/* WordPress admin chrome wrapper for BeepBeep AI */

const WPChrome = ({ children, activeTab, onTab, plan, onUpgrade, user, onSignOut, onHelp, signedOut = false }) => {
  const items = [
    { icon: "home-wp", label: "Dashboard", group: "core" },
    { icon: "pin-wp", label: "Posts", group: "core" },
    { icon: "media-wp", label: "Media", group: "core" },
    { icon: "page-wp", label: "Pages", group: "core" },
    { icon: "comment-wp", label: "Comments", group: "core" },
    { icon: "nai-wp", label: "BeepBeep AI", group: "active", isNai: true },
    { icon: "appearance-wp", label: "Appearance", group: "lower" },
    { icon: "plugin-wp", label: "Plugins", group: "lower" },
    { icon: "users-wp", label: "Users", group: "lower" },
    { icon: "tools-wp", label: "Tools", group: "lower" },
    { icon: "settings-wp", label: "Settings", group: "lower" },
    { icon: "collapse-wp", label: "Collapse menu", group: "footer" },
  ];

  // Primary nav — simplified 4-tab IA.
  // Home = daily ritual · Library = control · Autopilot = automation · Settings = utilities.
  // No Activity / Analytics / Usage at top level — these live ambient on Home.
  const naiTabs = [
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <aside style={{
        ...
        {items.map((it, i) => <WPNavItem key={i} {...it} />)}
      </aside>

      {/* Main */}
      <main style={{ marginLeft: 160, marginTop: 32, flex: 1, minWidth: 0, position: "relative" }}>
        {/* BeepBeep AI top tab strip — tighter grouping */}
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", marginRight: 22 }}>
            <span style={{ fontWeight: 700, fontSize: 14.5, letterSpacing: "-0.015em", color: "var(--text)" }}>BeepBeep AI</span>
          </div>
          <nav style={{ display: "flex", alignItems: "center", gap: 2, height: "100%" }}>
            {!signedOut && naiTabs.map(t => {
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
            {!signedOut && (
              <>
                {/* Status chip — muted, low-hierarchy. Doesn't compete with the primary CTA. */}
                <Tooltip content="BeepBeep AI is monitoring uploads in the background">
                  <span style={{
...
const WPTopBar = ({ plan }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 16, fontSize: 13, color: "#c3c4c7", width: "100%" }}>
    ...
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
      ...
      <span>WP-Alt-text-plugin</span>
    </span>
    <span style={{ opacity: 0.4 }}>·</span>
    <span style={{ opacity: 0.7 }}>💬 0</span>
    <span style={{ opacity: 0.7, display: "inline-flex", alignItems: "center", gap: 4 }}>+ New</span>
    <span style={{ marginLeft: "auto", opacity: 0.7 }}>Howdy, admin</span>
    <span style={{ width: 22, height: 22, borderRadius: 999, background: "#3c434a", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 11 }}>A</span>
  </div>
);

const WPNavItem = ({ icon, label, group, isNai }) => {
    ...
    <>
      ...
      <div style={{
        ...
        <span style={{ width: 16, fontSize: 13, display: "inline-flex", justifyContent: "center", opacity: isActive ? 1 : 0.85 }}>
          {isNai ? <Icon name="logo" size={14} style={{ color: "#fff" }}/> : glyphs[icon]}
        </span>
        <span>{label}</span>
      </div>
      {isActive && (
        <div style={{ background: "#2c3338", paddingTop: 4, paddingBottom: 4 }}>
          {["Home","Library","Autopilot","Settings"].map((l, i) => (
            <div key={l} style={{
              padding: "5px 12px 5px 30px", fontSize: 13,
              color: i === 0 ? "#fff" : "#bfc4ca",
