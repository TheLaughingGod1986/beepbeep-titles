/* BeepBeep Titles & meta Dashboard — daily page SEO assistant
...
const Dashboard = ({ state, plan, autoOptimise, onAutoToggle, onGenerate, onUpgrade, onView, lastVisit, streak }) => {
  ...
  const data = {
    fresh: { total: 412, optimised: 28, needsAttn: 384, lowQuality: 0, newSinceVisit: 0, coverage: 7 },
    mid:   { total: 412, optimised: 247, needsAttn: 142, lowQuality: 23, newSinceVisit: 3, coverage: 60 },
    near:  { total: 412, optimised: 389, needsAttn: 11, lowQuality: 12, newSinceVisit: 3, coverage: 94 },
  }[state];
  ...
  // Forward-looking 'steady improvement' projection. Assumes ~5 pages/day
  ...
  return (
    <div style={{ padding: "24px 32px 48px", maxWidth: 1180, margin: "0 auto" }}>
      ...
      {/* Site coverage — reinforces overall progress momentum */}
...
const TodaysPassHero = ({ data, plan, dailyUsed, dailyLimit, dailyRemaining, onGenerate, onUpgrade }) => {
  // Each page carries an "intelligence signal" — why it was picked.
  // Tone hierarchy:
  //   danger   = missing title (urgent / blocking)
  //   warn     = missing meta or needs review (actionable)
  //   neutral  = informational context (Homepage, High traffic, Cornerstone)
  const queuePages = [
    { url: "/about",                      section: "About",   hue: 220, sectionLetter: "A", signal: "Missing title",      tone: "danger"  },
    { url: "/blog/seo-guide-2026",        section: "Blog",    hue: 145, sectionLetter: "B", signal: "High traffic",       tone: "neutral" },
    { url: "/shop/ethiopia-light-roast",  section: "Shop",    hue: 30,  sectionLetter: "S", signal: "Missing meta",       tone: "warn"    },
    { url: "/reviews/jane-davis",         section: "Reviews", hue: 280, sectionLetter: "R", signal: "Needs review",       tone: "warn"    },
    { url: "/recipes/cappuccino-at-home", section: "Blog",    hue: 60,  sectionLetter: "B", signal: "Cornerstone",        tone: "neutral" },
  ];
  const queueCount = Math.min(dailyRemaining, data.needsAttn, 5);
  ...
  return (
    <div
      ...
      aria-label={interactive ? `Start today's pass — ${queueCount} pages ready` : undefined}
    ...
    >
          ...
          <div style={{ fontSize: 19, fontWeight: 600, letterSpacing: "-0.015em", lineHeight: 1.3 }}>
            {showQueue
              ? <>{data.newSinceVisit > 0
                    ? <><span className="mono tnum">{data.newSinceVisit}</span> new page{data.newSinceVisit === 1 ? "" : "s"} detected since your last scan</>
      ...
      {showQueue ? (
        <div style={{
          ...
          {queuePages.slice(0, queueCount).map((pg, i) => (
            <div key={i} style={{
              padding: 10,
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                <div style={{ marginTop: 3 }}>
                  <SignalChip tone={pg.tone}>{pg.signal}</SignalChip>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ padding: "10px 20px 0" }}>
          <div style={{
            ...
            <div style={{ flex: 1, fontSize: 13, color: "var(--ok-ink)", lineHeight: 1.45 }}>
              {plan === "pro"
                ? <><strong style={{ fontWeight: 600 }}>Optimisation complete.</strong> Autopilot is monitoring new pages in the background.</>
          ...
          {plan === "pro" ? (
            showQueue ? (
              <>
                ...
                <span>· <span className="mono tnum">{dailyUsed}</span> pages improved today</span>
              </>
            ) : (
              <>
                <Icon name="check" size={12} style={{ color: "var(--ok-ink)" }} strokeWidth={2.6}/>
                <span style={{ color: "var(--text-2)", fontWeight: 500 }}>
                  <span className="mono tnum">{dailyUsed}</span> pages improved today
                </span>
...
/* ----------------------------------------------------------------
   Site SEO health — calm progress card.
...
const LibraryHealthCard = ({ coverage, tone, stageLabel, nextMilestone, hoursSaved, totalGenerated, projectionTarget, weeksToTarget }) => {
  ...
  // Stage label retained for screen-reader / aria context. The card narrative
  // already communicates progress without rendering it as a chip.
  return (
...
/* ----------------------------------------------------------------
   Site coverage — calm progress module. Compact full-width strip
  ...
  <Card padding={0}>
    <div style={{ padding: "18px 22px 14px" }}>
      ...
      <div style={{ fontSize: 16, fontWeight: 600, letterSpacing: "-0.01em", marginBottom: 4 }}>
        Automatically optimise new pages in the background.
      </div>
    </div>
    <div style={{ padding: "2px 22px 14px", display: "flex", flexDirection: "column", gap: 6 }}>
      {[
        "New pages covered instantly",
    ...
    <div style={{ padding: "18px 22px 12px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12.5, color: "var(--text-2)", marginTop: 4 }}>
          {autoOptimise
            ? "New pages get title & meta the moment they're added."
            : "Re-enable to keep new pages covered automatically."}
        </div>
      </div>
...
const ActivityStrip = ({ onView }) => {
  ...
  const events = [
    { time: "2m ago",     icon: "upload",   tone: "warn",    text: "3 new pages detected in /blog",                action: "Review", onAction: () => onView && onView("library") },
    { time: "Yesterday",  icon: "sparkles", tone: "primary", text: "5 pages improved · coverage increased by 12%",  action: null },
  ];
  return (
    <div style={{
      marginTop: 22,
  ...
  if (plan === "pro") {
    return (
      <div style={{
        ...
        <Item label="Improvements this week" value={<><span className="mono tnum">{monthlyUsed}</span> pages · <span style={{ color: "var(--text-3)" }}>+18% vs last week</span></>}/>
        <Item label="Autopilot" value={
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
            <span className="pulse-dot" style={{ width: 6, height: 6, background: "var(--ok)" }}/>
            Active · <span style={{ color: "var(--text-3)" }}>monitoring new pages</span>
          </span>
        }/>
      </div>
    );
